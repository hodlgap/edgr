...
var (
	...
	validFileRegex = regexp.MustCompile(`.+(?i)\.(htm|html|txt)\z`)
)

// Get gets the documents.
func Get(url string) (docs []*model.Document, err error) {
	indexPg, err := request.GetPage(url)
	if err != nil {
		return nil, err
	}
...
func GetDocsFromTxt(textURL string) (docs []*model.Document, err error) {
	...
	textDocument, err := request.GetPage(textURL)
	if err != nil {
		return nil, err
	}
...
func parseFullText(url, fullText string) (docs []*model.Document, err error) {
	...
	for _, match := range matches {
		...
		isValid := validFileRegex.MatchString(filename)
		if !isValid && strings.Contains(filename, ".xml") {
			...
			htmlText, pageErr := request.GetPage(edgarURL)
			if pageErr != nil {
